...
function google_business_reviews_rating(e, i) {
	...
	jQuery('.google-business-reviews-rating').each(function(index) {
		...
		var e = jQuery(this),
			stars_html = (typeof jQuery(this).attr('class') == 'string' && (jQuery(this).hasClass('stars-html') || jQuery(this).attr('class').match(/\bversion[_-]?1\b/i))),
			star_css = (!stars_html && typeof jQuery(this).attr('class') == 'string' && jQuery(this).hasClass('stars-css')),
		...
		if (!stars_html && jQuery('.star', jQuery('.all-stars', e)).length) {
			if (star_css) {
		...
		if (!stars_html && jQuery('.all-stars', e).length && jQuery('.all-stars', e).hasClass('animate') && typeof rating == 'number' && rating > 1.5) {
			jQuery('.all-stars', e)
				.after(jQuery('<span>')
					.addClass('all-stars')
					.addClass('backdrop')
					.width(jQuery('.all-stars', e).width() + 1)
					.css({ 'margin': parseInt(jQuery('.all-stars', e).css('top')) + 'px 0 0 ' + (-1 * jQuery('.all-stars', e).width() - 0.1) + 'px' })
		...
		else if (stars_html && typeof rating == 'number') {
			if (safari) {
